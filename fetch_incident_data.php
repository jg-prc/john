<?php
	...
	$statuses = ['pending', 'ongoing', 'resolved', 'duplicated'];
	$incident_data = [
		'Vehicular' => '',
		'Fire' => '',
		'Flood' => '',
		...
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				...
				if (isset($incident_location[$barangay])) {
					if ($status === 'pending' || $status === 'duplicated') {
	...
	fetch_incidents($conn, $statuses[0], $type, $date, $incident_data, $incident_location, 'blink');       // Pending
	fetch_incidents($conn, $statuses[1], $type, $date, $incident_data, $incident_location, 'blink');       // Ongoing
	fetch_incidents($conn, $statuses[2], $type, $date, $incident_data, $incident_location, 'not-blink');   // Resolved
	fetch_incidents($conn, $statuses[3], $type, $date, $incident_data, $incident_location, '');            // Duplicated
