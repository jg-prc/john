					...
					<div class="swal-content">
						...
						<div class="swal-body">
							...
							<div class="input-container" id="row5">
								...
								<div class="btn">
									<button class="deactivate-btn" onclick="confirmDeactivation(${OfficialsID})">Deactivate</button>
								</div>
							</div>
						</div>
					</div>
...
function confirmDeactivation(OfficialsID) {
	Swal.fire({
		title: 'Are you sure?',
		...
		if (result.isConfirmed) {
			fetch(`php/deactivate.php?OfficialsID=${OfficialsID}`, {
				method: 'POST',
			})
			...
			.catch(error => {
				...
				Swal.fire(
					'Error!',
					'There was an error deactivating the account.',
					'error'
				);
			});
		}
