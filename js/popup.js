function showForm(user_id) {
	...
	fetch(`php/get_data.php?user_id=${user_id}`)
					...
					<div class="swal-content">
						...
						<div class="swal-body">
							...
							<div class="input-container" id="row5">
								...
								<div class="btn">
									<button class="deactivate-btn" onclick="confirmDeactivation(${user_id})">Deactivate</button>
								</div>
							</div>
						</div>
					</div>
...
function confirmDeactivation(user_id) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			fetch(`php/deactivate.php?user_id=${user_id}`, {
				method: 'POST',
			})
